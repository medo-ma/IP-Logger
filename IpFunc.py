...
#check if there's a need for resetting the config    
        if len(config_file) == 0:
            api_Qscore = []
            #api entering step
            api_Qx = int(input("how many IPQS-APIs do you want to add?\n"))
            ...
            if Notify:    
                with open('config.txt', 'w') as config_file:
                    config_file.write("#set up yor prefered configuration from here:\n")
...
def get_public_ip():
    try:
        if response := requests.get('https://api64.ipify.org?format=json', timeout=5):
            response.raise_for_status()
            return response.json().get('ip')
        elif response := requests.get("http://ip-api.com/json/",timeout=5):
            response.raise_for_status()
            return response.json().get("query")
        elif response := requests.get("https://ipapi.co/json/",timeout=5):
            response.raise_for_status()
            return response.json().get("ip")
    ...
    for interface, addresses in addrs.items():
        if interface in stats and stats[interface].isup:
            for address in addresses:
                if address.family == socket.AF_INET:  # Check for IPv4 address
                    return True
    return False
...
def log_ip(ip):
    try:
        ...
        ApiReq:list = str(api_Qscore[apiNum]).split('/')
...
def qu():
    time.sleep(3)
    if (input("#the script is working, press enter to exit\n")):
        quit()
